;(function(global, factory) {
	...
	var NOTHING = hasSymbol
		...
		: ((_ref = {}), (_ref["immer-nothing"] = true), _ref)
	/**
		...
		if (!state.finalized_) {
			...
			var result = // For ES5, create a good copy from the draft first, with added keys and without deleted keys.
				...
				state.type_ === 5
					...
					: state.copy_ // Finalize all children of the copy
		...
		if (isDraft(childValue)) {
			var path =
				...
				!has(parentState.assigned_, prop) // Skip deep patches for assigned keys.
					...
					: undefined // Drafts owned by `scope` are finalized here.
	...
	var objectTraps = {
		...
		set: function set(
			...
			if (!state.modified_) {
				...
				var currentState =
					current === null || current === void 0 ? void 0 : current[DRAFT_STATE]
	...
	function readPropFromProto(state, source, prop) {
		...
		return desc
			? "value" in desc
				...
				(_desc$get = desc.get) === null || _desc$get === void 0
				? void 0
				: _desc$get.call(state.draft_)
			: undefined
	}
	...
	function currentImpl(value) {
		...
		if (state) {
			if (
				...
				return state.base_ // Optimization: avoid generating new drafts during copying
		...
		function createES5Draft(isArray, base) {
			if (isArray) {
				...
				for (var _i = 0; _i < keys.length; _i++) {
					var key = keys[_i]
					_descriptors[key] = proxyProperty(
						key,
						isArray || !!_descriptors[key].enumerable
					)
				}
	...
	function enablePatches() {
		...
		function deepClonePatchValue(obj) {
			...
			if (isMap(obj))
				return new Map(
					Array.from(obj.entries()).map(function(_ref3) {
						var k = _ref3[0],
							v = _ref3[1]
	...
	function enableMapSet() {
		...
		function __extends(d, b) {
			...
			d.prototype = ((__.prototype = b.prototype), new __()) // @ts-ignore
		}
